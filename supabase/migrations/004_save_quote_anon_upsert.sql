-- Migration: Add save_quote_anon upsert RPC
-- Combines create + update into a single atomic operation
-- Eliminates the separate create/update paths for anonymous users
-- ============================================
...
CREATE OR REPLACE FUNCTION api.save_quote_anon(
  ...
  p_gst NUMERIC(10,2) DEFAULT 0,
  p_total NUMERIC(10,2) DEFAULT 0,
...
BEGIN
  ...
  INSERT INTO api.quotes (
    ...
    p_subtotal, p_gst, p_total, p_status
  )
...
GRANT EXECUTE ON FUNCTION api.save_quote_anon(UUID, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, JSONB, JSONB, TEXT, BOOLEAN, NUMERIC, NUMERIC, NUMERIC, NUMERIC, TEXT) TO anon, authenticated;
